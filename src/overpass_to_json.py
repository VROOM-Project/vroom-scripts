...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate problem from an overpass query"
    )
    parser.add_argument(
        "-c",
        "--city",
    ...
    if args.city is not None:
        ...
        nodes = node_coordinates_city(args.key, values, args.city)["elements"]
    else:
        if not file_name:
            file_name = (
                args.key
        ...
        nodes = node_coordinates_bb(
            args.key, values, [[args.left, args.bottom], [args.right, args.top]]
        )["elements"]
