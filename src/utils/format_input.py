...
def format_json_from_locations(locations):
    ...
    for i in range(len(locations["vehicles"]["coordinates"])):
        ...
        if ("names" in locations["vehicles"]) and (
            locations["vehicles"]["names"][i] is not None
        ):
            instance["vehicles"][-1]["startDescription"] = locations["vehicles"][
                "names"
            ][i]
            instance["vehicles"][-1]["endDescription"] = locations["vehicles"]["names"][
                i
            ]
    ...
    for i in range(j):
        ...
        if ("names" in locations["jobs"]) and (
            locations["jobs"]["names"][i] is not None
        ):
    ...
    for i in range(s):
        ...
        if "names" in locations["shipments"]:
            if locations["shipments"]["names"][2 * i] is not None:
                current["pickup"]["description"] = locations["shipments"]["names"][
                    2 * i
                ]
            if locations["shipments"]["names"][2 * i + 1] is not None:
                current["delivery"]["description"] = locations["shipments"]["names"][
                    2 * i + 1
                ]
...
def format_geojson_from_locations(locations):
    ...
    for key in locations:
        for i in range(len(locations[key]["coordinates"])):
            current = {
                "type": "Feature",
                "properties": {"id": i, "status": key, "name": key + " " + str(i)},
                "geometry": {
                    "type": "Point",
...
def write_files(file_name, locations, geojson, csv):
    ...
    with open(file_name + ".json", "w") as out:
        ...
        json.dump(json_input, out, indent=2)
    if geojson:
