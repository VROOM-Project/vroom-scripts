...
def parse_job(line, pickups, deliveries, coords):
  ...
  job = {
    ...
    'time_windows': [[CUSTOM_PRECISION * int(float(x[4])),
...
def parse_pdptw(input_file):
  ...
  for pickup in pickups:
    ...
    if ((delivery['matching_pickup'] != pickup['id']) or (delivery['amount'][0] != - pickup['amount'][0])):
  ...
  return {'meta': meta, 'vehicles': vehicles, 'shipments': shipments, 'matrix': matrix}
